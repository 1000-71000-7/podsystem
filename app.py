from flask import Flask, request, jsonify, render_template, session, g, redirect, url_for, flash, Response
...
def handle_connect():
    ...
    emit('connected', {'message': 'Connected', 'timestamp': datetime.utcnow().isoformat(), 'client_id': client_id})
...
def emit_metrics_update(site_id, metrics_data):
    try:
        ...
        socketio.emit('metrics_update', {
            'site_id': site_id, 'data': metrics_data, 'timestamp': datetime.utcnow().isoformat()
        }, room=room_name)
    ...
    def export_to_pdf(data, filename, title="Отчёт по метрикам"):
        try:
            ...
            title_style = ParagraphStyle('CustomTitle', parent=styles['Heading1'], fontSize=16,
                                         textColor=colors.HexColor('#1a73e8'), spaceAfter=30)
            story = []
    ...
    try:
        ...
        if event_type == 'metric_lcp':
            ...
            form = FormInteraction(site_id=site.id, session_id=data.get('session_id'),
                                   form_id=event_data.get('form_id'), start_time=datetime.utcnow())
            db.session.add(form)
            ...
            if last_form:
                last_form.submit_time = datetime.utcnow()
...
@app.route('/health')
def health_check():
    status = {'status': 'ok', 'timestamp': datetime.utcnow().isoformat(), 'version': '2.0.0', 'checks': {}}
    try:
...
def get_trends():
    try:
        ...
        if not aggregates:
            start_date_dt = datetime.utcnow() - timedelta(days=days)
...
def get_top_pages():
    ...
    try:
        ...
        ).group_by(Event.url).order_by(func.desc('count')).limit(limit).all()
...
def init_db():
    with app.app_context():
        try:
            ...
            init_admin_user()
            if Site.query.count() == 0:
                demo_site = Site(name="Демо сайт", url="https://example.com", tracker_id="demo_tracker", is_active=True)
                db.session.add(demo_site)
                db.session.commit()
                logger.info("Демо-сайт создан")
        except Exception as e:
...
if __name__ == '__main__':
    ...
    # ⚠️ ВАЖНО: Для локального запуска используем socketio.run()
    # Для продакшена на Timeweb будет использоваться gunicorn
    # Эта секция сработает ТОЛЬКО при прямом запуске python app.py
    socketio.run(
        app,
        debug=app.config.get('FLASK_DEBUG', False),  # Меняем True на False для безопасности
